...
class UpdateDistTest(unittest.TestCase):
    ...
    def make_repository(self, directory):
        ...
        shutil.copy2(CORE_DIRECTORY / "scripts/build_public_json.py", scripts)
        shutil.copytree(CORE_DIRECTORY / "scripts/lib", scripts / "lib")
    ...
    def test_success_replaces_dist(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                {
                    "title": "rule",
                    "rules": [{"description": "rule", "manipulators": []}],
                },
                __import__("json").loads(
                    (root / "dist/json/rule.json").read_text("utf-8")
                ),
            )
